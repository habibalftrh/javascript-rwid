...
document.getElementById("form").addEventListener("submit", function (event) {
  ...
  try {
    ...
    const resultObj = new Palindrome(word);
    let result;
    if (method === "reverse") {
      result = resultObj.isPalindromeUsingReverse(word);
    } else if (method === "loop") {
      result = resultObj.isPalindromeUsingLoop(word);
    } else if (method === "recursive") {
      result = resultObj.isPalindromeUsingRecursive(word);
    }
